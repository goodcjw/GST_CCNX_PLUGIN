...
gst_ccnx_packet_header_unpack (const struct ccn_charbuf * header_buf)
{
  ...
  gst_ccnx_unpack_be_guint_x (&header->mLeft, &header_buf->buf[0], 1);
  gst_ccnx_unpack_be_guint_x (&header->mOffset, &header_buf->buf[1], 2);
  gst_ccnx_unpack_be_guint_x (&header->mCount, &header_buf->buf[3], 1);
...
gst_ccnx_segment_header_unpack (const struct ccn_charbuf * header_buf)
{
  ...
  gst_ccnx_unpack_be_guint_x (&header->mSize, &header_buf->buf[0], 4);
  gst_ccnx_unpack_be_guint_x (&header->mTimestamp, &header_buf->buf[4], 8);
  gst_ccnx_unpack_be_guint_x (&header->mDuration, &header_buf->buf[12], 8);
